...
module.exports = {
  ...
  delete: async (req, res) => {
    try {
      ...
      const concertInfo = await Concerts.findOne({ where: { id: concertid } });
      // 존재하지 않는다면, 다음을 실행한다
      if(!concertInfo) return res.status(400).json({ message: '콘서트가 존재하지 않습니다!' })
      // 존재한다면, 다음을 실행한다
      else {
        ...
        if(!alarmInfo) return res.status(400).json({ message: '요청과 일치하는 콘서트가 존재하지 않습니다!' })
        // 만약 일치하는 알람 정보가 있다면, 다음을 실행한다
          ...
          if(alarm_type === 'email') {
            ...
            return res.status(201).json({ data: { alarmDestroyInfo: alarmDestroyInfo }, message: '이메일 알람 설정이 취소 되었습니다!'})
          }
          // 카카오톡 알람이면, 다음을 실행한다
          else if(alarm_type === 'kakao') {
            ...
            return res.status(201).json({ data: { alarmDestroyInfo: alarmDestroyInfo }, message: '카카오톡 알람 설정이 취소 되었습니다!'})
          }
        }
      }
