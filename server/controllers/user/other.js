  ...
  get: async (req, res) => {
    try {
      ...
      if (otherInfo.role === 3)
        return res
          .status(200)
          .json({ message: 'Can Not Search Other UserInfo!' });
      ...
      const transformInfo = {
        ...
        email: otherInfo.email
          ...
          .replace(
            new RegExp(
              '.(?=.{0,' +
                (otherInfo.email.toString().split('@')[0].length - 3) +
                '}@)',
              'g',
            ),
            '*',
        ...
        birth:
          today.getFullYear() -
          Number(otherInfo.birth.substring(0, 4)) +
          1 +
          '살',
        createdAt:
          otherInfo.createdAt.getFullYear() +
          '년 ' +
          Number(otherInfo.createdAt.getMonth() + 1) +
          '월 ' +
          otherInfo.createdAt.getDate() +
          '일',
      };
